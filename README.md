...
问答台可以切换“标准 Agent”和“协作研究”。标准 Agent 会写入当前会话，并兼容支持 `reasoning_content` 的模型：模型返回深度思考时，回答前会显示默认折叠的思考卡片；思考内容只保留在当前页面，不写入会话历史。协作研究会展示 Researcher 的检索轨迹与引用，但结果只保留在当前页面，不写入会话历史。
