...
const (
	...
	defaultAgentWorkerConcurrency       = 1
	...
	defaultModelCircuitFailureThreshold = 3
	defaultModelCircuitRecoveryTimeout  = 30 * time.Second
	...
	defaultSandboxMemoryBytes           = int64(512 * 1024 * 1024)
	defaultSandboxCPUs                  = 1.0
	defaultSandboxPIDs                  = 64
	defaultSandboxDiskBytes             = int64(100 * 1024 * 1024)
	defaultSandboxMaxOutputBytes        = 128 * 1024
	defaultSandboxMaxCodeBytes          = 64 * 1024
	defaultSandboxMaxConcurrent         = 2
...
type Config struct {
	...
	MultimodalEmbeddingAPIKey           string
	UploadDir                           string
	...
	ModelCircuitBreakerFailureThreshold int
	ModelCircuitBreakerRecoveryTimeout  time.Duration
	...
	SandboxEnabled                      bool
	SandboxDockerBinary                 string
	SandboxImage                        string
	SandboxTimeout                      time.Duration
	SandboxMemoryBytes                  int64
	SandboxCPUs                         float64
	SandboxPIDs                         int
	SandboxDiskBytes                    int64
...
func Load() Config {
	...
	return Config{
		...
		MultimodalEmbeddingBaseURL:          strings.TrimSpace(os.Getenv("MULTIMODAL_EMBEDDING_BASE_URL")),
		MultimodalEmbeddingModel:            strings.TrimSpace(os.Getenv("MULTIMODAL_EMBEDDING_MODEL")),
		MultimodalEmbeddingAPIKey:           strings.TrimSpace(os.Getenv("MULTIMODAL_EMBEDDING_API_KEY")),
		UploadDir:                           uploadDir(),
		...
		AgentWorkerConcurrency:              positiveIntEnv("AGENT_WORKER_CONCURRENCY", defaultAgentWorkerConcurrency),
		...
		ModelCircuitBreakerFailureThreshold: positiveIntEnv("MODEL_CIRCUIT_BREAKER_FAILURE_THRESHOLD", defaultModelCircuitFailureThreshold),
		ModelCircuitBreakerRecoveryTimeout:  durationEnv("MODEL_CIRCUIT_BREAKER_RECOVERY_TIMEOUT", defaultModelCircuitRecoveryTimeout),
		...
		SandboxEnabled:                      boolEnv("SANDBOX_ENABLE", false),
		SandboxDockerBinary:                 valueOrDefault("SANDBOX_DOCKER_BIN", "docker"),
		SandboxImage:                        valueOrDefault("SANDBOX_IMAGE", "python:3.12-alpine"),
		SandboxTimeout:                      durationEnv("SANDBOX_TIMEOUT", defaultSandboxTimeout),
		SandboxMemoryBytes:                  positiveInt64Env("SANDBOX_MEMORY_BYTES", defaultSandboxMemoryBytes),
		SandboxCPUs:                         positiveFloat64Env("SANDBOX_CPUS", defaultSandboxCPUs),
		SandboxPIDs:                         positiveIntEnv("SANDBOX_PIDS_LIMIT", defaultSandboxPIDs),
		SandboxDiskBytes:                    positiveInt64Env("SANDBOX_DISK_BYTES", defaultSandboxDiskBytes),
		SandboxMaxOutputBytes:               positiveIntEnv("SANDBOX_MAX_OUTPUT_BYTES", defaultSandboxMaxOutputBytes),
		SandboxMaxCodeBytes:                 positiveIntEnv("SANDBOX_MAX_CODE_BYTES", defaultSandboxMaxCodeBytes),
		SandboxMaxConcurrent:                positiveIntEnv("SANDBOX_MAX_CONCURRENT", defaultSandboxMaxConcurrent),
...
func positiveFloat64Env(name string, fallback float64) float64 {
