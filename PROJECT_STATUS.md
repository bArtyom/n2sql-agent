...
- 最新工具审批切片：标准 Agent 在工具真正执行前发送 `approval_required` SSE 事件并暂停；前端确认卡片调用 `POST /api/knowledge-bases/{id}/agent-runs/{runID}/approval` 恢复或拒绝运行；审批状态按知识库 ID 和运行 ID 隔离，取消/结束时释放；当前为进程内 Hub，Redis/持久化审批后置。
