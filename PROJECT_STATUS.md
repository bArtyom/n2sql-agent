...
- 最新提交：`dda9328 feat: add conversation search`（前置功能提交为 `ef60f95`、`13f3867`、`3333c11`）。
- 当前工作区在该提交后保持干净。
...
下一轮候选：对照盘点小/中切片已完成（#1 停止生成、#2 置顶分组、#3 自动标题、#4 换一批、#7 更多菜单、#5 正文引用、#9 断流续传、#8 消息分页、#6 实时检索进度、#12 工具结果类型化渲染、#11 会话内模型切换、#13 深度思考展示、#10 聊天附件/图片、#14 会话搜索）。剩余：Web 搜索（#15，边界外）、工具审批（#16，后置）。建议下一步在真实 Provider 环境验收思考参数、视觉模型和会话搜索，再评估供应商专属 thinking budget。
