...
- 最新提交：`29ad954 feat: render inline citation markers in answers`。
- 当前工作区在该提交后保持干净。
...
下一轮候选：对照盘点小/中切片已全部完成（#1 停止生成、#2 置顶分组、#3 自动标题、#4 换一批、#7 更多菜单、#5 正文引用）。剩余：#9 断流续传（刷新后恢复未完成回答，与 #1 共享 hub 机制）、#8 历史消息游标分页、#6 实时 RAG 流水线进度（中规模）；更大项（会话搜索、Web 工具、审批）在边界外或后置。
