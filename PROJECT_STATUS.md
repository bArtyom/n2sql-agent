...
- 最新工具审批切片：标准 Agent 在工具真正执行前发送 `approval_required` SSE 事件并暂停；前端确认卡片可编辑 JSON 参数，并调用 `POST /api/knowledge-bases/{id}/agent-runs/{runID}/approval` 恢复或拒绝运行；后端限制修改参数 32KB、重新校验 JSON 后才交给工具；审批状态按知识库 ID 和运行 ID 隔离，取消/结束时释放；默认 30 秒未确认会发送 `approval_expired` 并结束本轮；当前为进程内 Hub，Redis/持久化审批后置。
