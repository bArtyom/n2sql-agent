...
## Agent 节点循环与审批中断恢复（2026-08-29）

- 用户问题：Agent 的节点循环是怎样转移的，以及工具需要审批时，中断后如何回到原来的工具调用位置。
- 当前 Agent 的核心是 ReAct 状态机，而不是线性流程：`model → tool → model` 可以循环；模型没有返回 `tool_calls` 时走 `model → finish`；工具需要审批或等待子 Agent 时走 `tool → interrupt`；不可恢复错误则结束 Run。
- `model` 节点负责调用模型并决定下一步；`tool` 节点负责执行模型已决定的工具；`interrupt` 节点负责持久化暂停原因；`finish` 节点表示最终答案已经生成。
- `internal/agentruntime/graph.go` 当前主要负责登记合法节点和校验 checkpoint 游标；真正的模型调用、工具调用和循环由 `internal/agentruntime/engine.go` 的 `RunWithEvents` 驱动。`current_node` 是粗粒度游标，`pending_tool_calls` 才是恢复具体工具调用的关键。
- 每次模型决定调用工具后，先把 assistant 的 `tool_calls` 和 `pending_tool_calls` 写入统一 checkpoint；工具执行成功后追加 `role=tool` 的结果，并把游标推进回 `model`，让模型判断是继续调用工具还是直接回答。
- 审批工具的持久化流程：模型决定调用 `write_file(call-9)` → 保存 `current_node=interrupt`、`pending_tool_calls` 和 `interrupt` 详情 → Worker 返回 `ErrAgentApprovalPending` → Runner 调用 `MarkWaitingApproval`，把 `agent_runs.status` 改成 `waiting_approval` 并释放租约。Worker 不会一直占用 goroutine 等待用户。
- 用户审批接口调用 `ResolveApproval(run_id, knowledge_base_id, approved)`；数据库事务会锁定 Run 和最新 checkpoint，检查 Run 仍为 `waiting_approval`，批准时删除 `interrupt` 并追加 `approved_tool_call_ids`，拒绝时追加 `rejected_tool_call_ids`，随后把 Run 重新排队为 `pending`，同时递增 checkpoint `version`。
- 下一次可以由任意 Worker 领取同一个 `run_id`。它读取统一 checkpoint：如果发现 `pending_tool_calls` 不为空，Engine 将其作为已经完成的模型决策，跳过再次调用模型；批准的 `call-9` 直接执行原工具，工具完成后保存结果并回到 `model`。因此逻辑路径是 `interrupt → tool → model → finish`。
- 如果用户拒绝，Worker 不执行副作用工具，而是把“用户拒绝了这个工具调用”作为该 `tool_call_id` 的工具观察结果交给模型，逻辑路径为 `interrupt → model`，模型可以重新规划。
- 审批恢复依赖同一个 `run_id` 和 checkpoint，不会创建新的会话或普通聊天轮次；Worker 接管时只会更换 `attempt_count`、`execution_id` 和 `lease_token`。`conversation_id` 仍然标识整个多轮会话。
- 设计原则：同一个 Run 的崩溃恢复必须保留完整的 assistant tool call、tool result、pending tool 和 interrupt；新一轮普通会话是否复用这些内部消息，则必须和“当前 Run 恢复”区分，不能因为普通会话表只保存 user/assistant 就误以为模型一定看不到 checkpoint 中的工具消息。
